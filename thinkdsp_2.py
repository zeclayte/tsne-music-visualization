...
class Wave:
    ...
    def make_spectrum_2(self, full=False):
        """Computes the spectrum using FFT.

        full: boolean, whethere to compute a full FFT
              (as opposed to a real FFT)

        returns: An array instead of an Spectrum object
        """
        n = len(self.ys)
        d = 1 / self.framerate
    ...
    def make_spectrogram_2(self, seg_length, win_flag=True):
        """Computes the spectrogram of the wave.
        ...
        while j < len(self.ys):
            ...
            t = (segment.start + segment.end) / 2
            if i==0:
                a = segment.make_spectrum_2()
            else:
                a = np.vstack([a,segment.make_spectrum_2()])
